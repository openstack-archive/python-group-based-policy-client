...
class UpdateExternalPolicy(CreateExternalPolicy):
...
class UpdateExternalSegment(CreateExternalSegment):
...
class CreateNatPool(neutronV20.CreateCommand):
    ...
    def add_known_arguments(self, parser):
        ...
        parser.add_argument(
            '--external-segment-id',
            help=_('UUID of External Segment'))
        parser.add_argument(
            '--shared', type=bool,
            help=_('Shared flag'))
    ...
    def args2body(self, parsed_args):
        ...
        if parsed_args.external_segment_id:
            neutronV20.find_resourceid_by_name_or_id(
                self.get_client(), 'external_segment',
                parsed_args.external_segment_id)
...
class UpdateNatPool(CreateNatPool):
